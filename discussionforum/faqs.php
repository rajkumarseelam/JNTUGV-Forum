				...
				<div class="card">
					...
					<div class="card-body">
						<ul class="w-100 list-group" id="topic-list">
							...
							endwhile;
							$topic = $conn->query("SELECT t.*,u.name,u.type FROM topics t inner join users u on u.id = t.user_id where frequent=1 order by unix_timestamp(date_created) desc");
							...
							<li class="list-group-item mb-4">
								...
								<p class="row justify-content-left"><span class="badge badge-success text-white"><i>Posted By: <?php echo $row['name'] ?> <?php echo ($row['type'] == 2)? '<b style="color:yellow">staff</b>' :''?></i></span></p>
								<hr>
								...
								<span class="float-left badge badge-primary text-white ml-2" ><i class="ri-thumb-up-fill"></i> <?php echo $row['likes'] ?> like/s  </span>
								<span class="float-left badge badge-primary text-white ml-2" ><i class="ri-thumb-down-fill"></i> <?php echo $row['dislikes']  ?> dislike/s  </span>
								<span class="float-right">
									<span>Tags: </span>
								<?php 
								foreach(explode(",",$row['category_ids']) as $cat):
								?>
								<span class="badge badge-info text-white ml-2"><?php if(isset($tags[$cat]))echo $tags[$cat]; else echo "Others";  ?></span>
							<?php endforeach; ?>
								</span>
							</li>
						<?php endwhile; ?>
